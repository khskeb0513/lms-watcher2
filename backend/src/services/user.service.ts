...
export class UserService {
  ...
  public async getNoticeList(cookie: string) {
    ...
    for (const v of response) {
      data.push({
        ...v,
        board: await this.boardService.getNoticeListByEClassId(v.id, cookie)
      });
    }
    return data;
  }
  ...
  public async getMaterialList(cookie: string) {
    ...
    for (const v of response) {
      data.push({
        ...v,
        board: await this.boardService.getMaterialListByEClassId(v.id, cookie)
      });
    }
    return data;
  }
  ...
  public async requestHisStatus(
    ...
    const his = await this.databaseService.getHisByUsernameItem(
      await this.getUsername(cookie),
      item
    );
    return {
      item,
  ...
  public async getUsername(cookie: string) {
    const response = await got.get(
      "https://lms.pknu.ac.kr/ilos/mp/myinfo_form.acl",
      {
        headers: {
          cookie
        }
    ...
    const str = $(
      "#uploadForm > div:nth-child(5) > table > tbody > tr:nth-child(1) > td:nth-child(2)"
    ).html();
  ...
  public async getSchedule(cookie: string, ...term: number[]) {
    const courseArr =
      term.length === 0
        ? await this.eClassService.getList(cookie)
        : await this.eClassService.getList(cookie, term[0], term[1]);
    const data = [];
    for (let i = 0; i < courseArr.length; i++) {
      data.push({
        ...courseArr[i],
