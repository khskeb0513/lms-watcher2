...
export class SessionService {
  public async isUser(username: string, password: string) {
    ...
    const cookie = cookieStr
      .slice(cookieStr.indexOf("JSESSIONID"), cookieStr.length)
      .split(";")[0];
  ...
  public async isSessionValid(cookie: string) {
    const request = await got.get(
      "https://lms.pknu.ac.kr/ilos/mp/user_image_view.acl",
      {
        headers: {
          cookie
        }
  ...
  public async moveKj(cookie: string, kjKey: string) {
    const request = await got.post(
      "https://lms.pknu.ac.kr/ilos/st/course/eclass_room2.acl",
      {
        headers: { cookie },
        searchParams: {
          KJKEY: kjKey,
  ...
  public getSessionFromEnv() {
    return process.env["COOKIE_STR"].replace(/\t/gi, "").replace(/ /gi, "");
  }
}
