...
export class ScheduleService {
  ...
  public async getAttendanceById(id: string, cookie: string) {
    const response = (await this.sessionService.moveKj(cookie, id))
      ? await got.get(
          "https://lms.pknu.ac.kr/ilos/st/course/attendance_list.acl",
          {
            headers: { cookie },
            searchParams: {
          ...
          schedules: body(v)
            ...
            .map((v) => {
              const progressStr = [
                ...
                body(v)
                  .contents()
                  .filter((i, e) => e.type === ElementType.Text)
                  .text()
                  .trim()
              ]
                .join(" ")
                .split("(");
              const name = progressStr
                .slice(0, progressStr.length - 1)
                .join("");
  ...
  public async getByEClassId(id: string, cookie: string) {
    const body = (await this.sessionService.moveKj(cookie, id))
      ? await got.get("https://lms.pknu.ac.kr/ilos/st/course/online_list.acl", {
          headers: { cookie }
        })
      : null;
    ...
    let scheduleArr = $("span.site-mouseover-color")
      ...
      .map((v) => {
        const progress = $(
          $(v).parent().parent().parent().children("div").toArray()[1]
        );
  ...
  public async getByEClassIdExceptComplete(id: string, cookie: string) {
    return (await this.getByEClassId(id, cookie)).filter(
      (v) => v.percent !== "100%"
    );
  }
  ...
  public async requestHisStatus(
    ...
    const response = await got.post(
      "https://lms.pknu.ac.kr/ilos/st/course/online_view_status.acl",
      {
        form: {
          lecture_weeks: week,
          item_id: item,
  ...
  public async getVideo(
    ...
    const formAcl = (await this.sessionService.moveKj(cookie, kjKey))
      ? await got.get(
    ...
    await this.databaseService.setContent(
      kjKey,
      itemId,
      naviAclBody["cid"],
      naviAclBody["content_id"]
    );
    return JSON.parse(naviAcl.body);
  }

  public async issueHisCode(itemId, week, kjKey, cookie: string) {
    const ud = await this.userService.getUsername(cookie);
    const video = (await this.sessionService.moveKj(cookie, kjKey))
      ? await this.getVideo(kjKey, week, itemId, cookie)
      : null;
  ...
  public async getHisCode(itemId, seq, kjKey, ud, cookie: string) {
    const dbResponse = await this.databaseService.getHisByUsernameItem(
      ud,
      itemId
    );
    if (dbResponse) {
