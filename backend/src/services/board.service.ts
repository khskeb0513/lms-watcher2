...
export class BoardService {
    ...
    public async getNoticeListByEClassId(id: string, cookie: string) {
        const response = await this.sessionService.moveKj(cookie, id) ? await got.get('https://lms.pknu.ac.kr/ilos/st/course/notice_list.acl', {
            headers: {cookie},
            searchParams: {
    ...
    public async getBody(id: string, url: string, attachmentId: string, cookie: string) {
        const response = await this.sessionService.moveKj(cookie, id) ?
            await got.get(`https://lms.pknu.ac.kr/${url}`, {headers: {cookie}}) : null
        const eFileResponse = await got.post('https://lms.pknu.ac.kr/ilos/co/efile_list.acl', {
            headers: {cookie},
            form: {
                ud: await this.userService.getUsername(cookie),
                ky: id,
                pf_st_flag: 2,
                CONTENT_SEQ: attachmentId ? attachmentId : response.body.split('CONTENT_SEQ : "')[1].split('"')[0],
                encoding: 'utf-8'
            }
        })
